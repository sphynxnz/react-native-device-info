...
public class RNDeviceModule extends ReactContextBaseJavaModule {
  ...
  * which is related to an issue with generating unique Secure.ANDROID_ID as described in the followingn
  ...
  * If an exception if encountered, the UUID falls back to just generating a random UUID version 4. 
  ...
  * a of lame attempt to align with the KeyChain approach use din iOS - the generated UUID could have easily
  * been returned from getUUIDfromAndroidId(). 
  */
  private String getLNZUUID() {
    String alias = "lnz-uuid";
    String uuid = getUUIDfromAndroidId();
    try {
      KeyStore keystore = KeyStore.getInstance("AndroidKeyStore");
      keystore.load(null);      // Create new key if needed
